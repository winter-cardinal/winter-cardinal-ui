...
export const toScaleInvariant = (style: EShapeStrokeStyle): number => {
	const nonExpandingWidth = style & EShapeStrokeStyle.NON_SCALING_WIDTH_WHEN_EXPANDING ? 1 : 0;
	const nonShrinkingWidth = style & EShapeStrokeStyle.NON_SCALING_WIDTH_WHEN_SHRINKING ? 2 : 0;
	const nonScalingDotAndDash = style & EShapeStrokeStyle.NON_SCALING_DOT_AND_DASH ? 4 : 0;
	if (style & (EShapeStrokeStyle.DASHED_MASK | EShapeStrokeStyle.DOTTED_MASK)) {
