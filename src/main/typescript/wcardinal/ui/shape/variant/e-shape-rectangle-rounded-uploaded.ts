...
export class EShapeRectangleRoundedUploaded extends EShapeTextUploaded {
	static readonly VERTEX_COUNT = 36;
	static readonly INDEX_COUNT = 24;
	...
	update( shape: EShape ): void {
		const buffer = this.buffer;
		this.updateRectangleRoundedVertexClippingStepAndUv( buffer, shape );
		this.updateColor( buffer, shape );
		this.updateText( buffer, shape );
	}

	protected updateRectangleRoundedVertexClippingStepAndUv( buffer: EShapeBuffer, shape: EShape ) {
		...
		if( isVertexChanged || isTransformChanged || isCornerChanged || isTextureChanged ) {
			...
			// Vertices
			const worldSize = EShapeRectangleRoundedUploaded.WORLD_SIZE;
	...
	protected doUpdateRectangleRoundedVertex(
		...
		// World size
		worldSize[ 0 ] = this.calcLength( x0, y0, x1, y1 );
		worldSize[ 1 ] = this.calcLength( x0, y0, x2, y2 );
		worldSize[ 2 ] = this.calcLength( x0, y0, x10, y10 );
	...
	protected doUpdateRectangleRoundedClipping(
	...
	protected doUpdateRectangleRoundedStep(
		steps: Float32Array,
		...
		this.calcStep( worldSize[ 1 ], strokeWidth, workStep );
		const swx = workStep[ 0 ];
		const px0 = workStep[ 1 ];
		const px1 = workStep[ 2 ];

		this.calcStep( worldSize[ 2 ], strokeWidth, workStep );
	...
	protected doUpdateRectangleRoundedUv(
		uvs: Float32Array,
		voffset: number,
