...
const toS = (target: number): string => {
...
export class UtilHtmlElementClipperExImpl implements UtilHtmlElementClipperEx {
	...
	protected toD(
		...
		for (let i = 0; i < rectsLength; i += 4) {
			...
			const d2 = rects[i + 2];
			const d3 = rects[i + 3];
			const x0 = toS(a * d0 + c * d1 + tx);
			const y0 = toS(b * d0 + d * d1 + ty);
			const x1 = toS(a * d2 + c * d1 + tx);
			const y1 = toS(b * d2 + d * d1 + ty);
			const x2 = toS(a * d2 + c * d3 + tx);
			const y2 = toS(b * d2 + d * d3 + ty);
			const x3 = toS(a * d0 + c * d3 + tx);
			const y3 = toS(b * d0 + d * d3 + ty);
