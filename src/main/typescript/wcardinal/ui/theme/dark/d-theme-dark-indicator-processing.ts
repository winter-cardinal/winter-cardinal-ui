...
const newProcessingTexture = () => {
	const resolution = (window.devicePixelRatio || 1) * 2;
	const size = 24 * resolution;
	return toSvgTexture(
		`<svg xmlns="http://www.w3.org/2000/svg" width="${size}px" height="${size}px" viewBox="0 -960 960 960">` +
...
let processingTexture: Texture | undefined;

const imageSource = (state: DBaseStateSet): Texture | DisplayObject | null => {
	if (state.isProcessing) {
		return (processingTexture ??= newProcessingTexture());
